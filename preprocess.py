...
def get_dataset(root: str = DATASETS_PATH, train_size: float = .8, shuffle=True, seed=None) -> AttributeDict:
    ...
    if train_size < 1:
        dataset.train.X, dataset.validation.X, dataset.train.y, dataset.validation.y = train_test_split(dataset.train.X,
                                                                                                        dataset.train.y,
                                                                                                        train_size=train_size,
                                                                                                        shuffle=shuffle,
                                                                                                        random_state=seed)
    else:
        dataset.validation.X = pd.DataFrame({c: [] for c in dataset.train.X.columns.values})
        dataset.validation.y = pd.DataFrame({c: [] for c in dataset.train.y.columns.values})
...
MRMR_5_FEATURES = ['kw_avg_avg', 'max_positive_polarity', 'weekday_is_wednesday', 'max_negative_polarity',
                   'abs_title_subjectivity']
MRMR_10_FEATURES = ['kw_avg_avg', 'max_positive_polarity', 'weekday_is_wednesday', 'max_negative_polarity',
                    'abs_title_subjectivity', 'data_channel_is_socmed', 'self_reference_min_shares',
                    'rate_negative_words', 'data_channel_is_lifestyle', 'weekday_is_saturday']
MRMR_15_FEATURES = ['kw_avg_avg', 'max_positive_polarity', 'weekday_is_wednesday', 'max_negative_polarity',
                    'abs_title_subjectivity', 'data_channel_is_socmed', 'self_reference_min_shares',
                    'rate_negative_words', 'data_channel_is_lifestyle', 'weekday_is_saturday', 'LDA_01', 'num_imgs',
                    'weekday_is_sunday', 'num_videos', 'kw_min_max']
MRMR_20_FEATURES = ['kw_avg_avg', 'max_positive_polarity', 'weekday_is_wednesday', 'max_negative_polarity',
                    'abs_title_subjectivity', 'data_channel_is_socmed', 'self_reference_min_shares',
                    'rate_negative_words', 'data_channel_is_lifestyle', 'weekday_is_saturday', 'LDA_01', 'num_imgs',
                    'weekday_is_sunday', 'num_videos', 'kw_min_max', 'kw_min_min', 'n_tokens_title',
                    'weekday_is_friday', 'data_channel_is_bus', 'min_positive_polarity']

RUN_FEATURES = {"all_features": None, "5_features": MRMR_5_FEATURES, "10_features": MRMR_10_FEATURES,
                "15_features": MRMR_15_FEATURES, "20_features": MRMR_20_FEATURES}
